...
PostProcessor::PostProcessor(Shader shader, GLuint width, GLuint height)
    ...
    glRenderbufferStorageMultisample(GL_RENDERBUFFER, 8, GL_RGB, width, height);                 // 为渲染缓冲区对象分配存储空间
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, this->Texture.ID, 0);   // 将纹理作为其颜色附加到framebuffer
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
